...
Web of Science (ďalej len WoS) je platená služba umožňujúca on-line prístup ku
...
\item \textbf{Conference Proceedings Citation Index\R\ -- Science (CPCI-S)}
  ...
  konferencií z~256 rôznych oblastí \uv{vedy a~techniky (CPCI-S)}
  a~\uv{sociálnych a~humanitných vied (CPCI-SSH)} od roku 1990.  Každý rok do
...
\item \emph{Analýza nesprávnych dokumentov} vzniká pretože žiadny algoritmus nie
  ...
  dokument, alebo ak zadané meno autora je bežné slovo (napr.~\uv{Lesný}).  Niekedy
...
autorov h-core článkov\index{h-core články}
...
Autori programu PoP\index{Publish or Perish} si vytvorili variant individuálneho
